...
export default function SingleSelectPanel({ hitl, onSubmit, isSubmitting, canSubmit = true }) {
  const options = hitl?.options || [];
  const [selected, setSelected] = useState(options[0]?.id || '');
  const [comment, setComment] = useState('');

  const title = hitl?.title || 'Choose one option';
    ...
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
      ...
      >
        ...
        <Button
          variant="contained"
          size="small"
          disabled={isSubmitting || !selected || !canSubmit}
